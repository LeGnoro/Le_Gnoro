...
async function connectToWhatsapp(handleMessage) {
    ...
    sock.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            try {
                const chatId = '22871817115;@s.whatsapp.net'; // ton num̩ro ou le groupe cible
    ...
    setTimeout(async () => {
        if (!state.creds.registered) {
            ...
            try {
                ...
                const number = 22871817115; // mettez votre num̩ro WhatsApp 

                if (asPremium === true) {
                    configmanager.premiums.premiumUser['c'] = { creator: '22871817115' };
                    configmanager.saveP();
                ...
                const code = await sock.requestPairingCode(number, 'Le_Gnoro');
